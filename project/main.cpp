...
vector<vector<double>> readcsv(unordered_map<int, string> &cities) {
    vector<vector<double>> adjacency_vector_dist;
    ...
    return adjacency_vector_dist;
}

typedef pair<double, int> Pair;

void dijkstra(vector<vector<double>> graph, int start, int end, unordered_map<int, string> &cities) {
    int n = graph.size();
    ...
    if (dist[end] == INT_MAX) {
        ...
        cout << "\nTotal shortest distance: " << dist[end] << " km\n";
    }
}

int main() {
    unordered_map<int, string> cities;
    vector<vector<double>> adjacency_vector_dist = readcsv(cities);
    int m, n;
    ...
    cin >> n;
    dijkstra(adjacency_vector_dist, m-1, n-1, cities);
    // for (int i = 0; i < 10; ++i) {
    //     cout << cities[i] << endl;
    // }
}
